...
@Component({
  selector: 'vex-data-type-interface',
  templateUrl: './data-type-interface.component.html',
  styleUrls: ['./data-type-interface.component.scss']
})
export class DataTypeInterfaceComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public defaults: any,
    ...
    private snackbar: MatSnackBar,private DataTypeSetupApi:DataTypeSetupService
    ) { }
  ...
  ngOnInit() {
    if (this.defaults) {
      ...
      this.defaults = {} as DataTypeModel;
    }

    this.form = this.fb.group({
      uniqueId: this.defaults.uniqueId || '',
      dataType: [this.defaults.dataType ||'', [Validators.required]],
      maxLength: [ this.defaults.maxLength ||'', [Validators.required]],
      minLength: [ this.defaults.minLength ||'', [Validators.required]],
  ...
  onSubmit() {
    this.submitted = true;
    if (!this.form.valid) {
      this.snackbar.open('Please fill all the required fields to Setup Data Type Info!', null, {
  ...
  GetComponentMasterByComponentID() {
    this.ComponentMasterApi.GetComponentMasterByComponentId('data-type-setup')
      .subscribe(res => {
  ...
  updateRoleMaster() {
    const message = this.form.value;
    this.dialogRef.close(message);
    this.getDataTypeInfoById(this.form.value.uniqueId);
  }
  ...
  /* ComponentMaster Get by uniqueId
    URL :  */
  getDataTypeInfoById(uniqueId: any) {
    this.DataTypeSetupApi.GetDataTypeByUniqueId(uniqueId)
      .subscribe(res => {
